...
function saveTraveler() {
    if (travelerList.length != 0) {
        ...
        xhttp.open("POST", "http://localhost:8081/TourWeb_war_exploded/api/controller/addTravelers", true);
...
function getTravelers() {
    if (!isRetrieved) {
        ...
        xhttp.open("GET", "http://localhost:8081/TourWeb_war_exploded/api/controller/getTravelers", true);
        xhttp.send();
        isRetrieved = true;
    }
    toggleGetTravelers();
...
function removeTraveler() {
    ...
    if (passport == null || passport == "") {
        ...
        let xhttp = new XMLHttpRequest();
        xhttp.open("DELETE", "http://localhost:8081/TravelWeb_war_exploded/api/controller/deleteTraveler", true);
        xhttp.onreadystatechange = function() {
            if (xhttp.readyState > 3 && xhttp.status == 200) {
